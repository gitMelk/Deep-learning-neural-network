...
#%% Training cycle
training = False ############################## Change to True for training
num_epochs = 20
best_loss = 1000
params_best = [0, 0]
counter = 0
...
if training:
  ...
  # Grid-search
  for encoded_space_dim in [2, 4, 6]:
    for lrate in [0.01, 0.001]:  
            counter = counter + 1
            params = [encoded_space_dim, lrate]
            loss_temp = []
            ...
            net.to(device)
            # Generating k-fold
            temp_kfold = kFolded(train_dataset);
            kfold_counter = 0
            guard = True
            # K-fold loop
            for trainloader, valloader in temp_kfold:
                if(guard):
                    for epoch in range(num_epochs):
                        ...
                        ### Training
                        train_epoch(net, dataloader=trainloader, loss_fn=loss_fn, optimizer=optim, print_loss = False) 
                        
                        if(epoch == num_epochs-1):
                            ### Validation
                            val_loss = test_epoch(net, dataloader=valloader, loss_fn=loss_fn, optimizer=optim) 
                            loss_temp.append(val_loss)
                            # Print Validationloss
                            kfold_counter = kfold_counter + 1
                            print('\n#Network #: ' + str(counter) + ' / ' + str(kfold_counter))
                            print('Current params: ' + str(params))
                            print('EPOCH %d/%d' % (epoch + 1, num_epochs))
                            print('\t VALIDATION - EPOCH %d/%d - loss: %f\n' % (epoch + 1, num_epochs, val_loss))
                            # Stop the loop if bad values
                            if(val_loss > batch_loss_max):
                                loss_temp.append(1000)
                                guard = False
                            break
                    loss_mean = np.mean(np.array(loss_temp))
...
if training:
    ...
    for epoch in range(num_epochs):
        ...
        # plt.pause(0.1)
        # Save figure
        os.makedirs('autoencoder_progress_%d_features_final' % encoded_space_dim, exist_ok=True)
        plt.savefig('autoencoder_progress_%d_features_final/epoch_%d.png' % (encoded_space_dim, epoch + 1))
        plt.show()
        plt.close()
        
        # After #50 epoch, if the loss does not improve for 5 consecutive times, stop 
        if(epoch > 50):
            if(val_loss < vall_loss_tmp):
                vall_loss_tmp = val_loss
                counter = 1
                torch.save(net.state_dict(), 'autoencoder_progress_6_features_final.pth')
            else:
                counter = counter + 1
            ...
            if(counter == 5):
                ...
                print('Epoch of the min: ' + str(epoch-5))
                print('Last min loss ' +str(vall_loss_tmp))
                break
...
for sample in tqdm(test_dataset):
    ...
    net.eval()
    with torch.no_grad():
        encoded_img  = net.encode(img.to(device))
    # Append to list
    encoded_samples.append((encoded_img.to("cpu").flatten().numpy(), label))
...
if encoded_space_dim == 2:
    ...
    encoded_value = torch.tensor([-1, -1]).float().unsqueeze(0)
    ...
    plt.show()
#%% Simple tests to check the quality of the network
net.load_state_dict(torch.load('autoencoder_progress_6_features_final.pth', map_location=device))
net.to(device)
val_loss = test_epoch(net, dataloader=test_dataloader, loss_fn=loss_fn, optimizer=optim) 
