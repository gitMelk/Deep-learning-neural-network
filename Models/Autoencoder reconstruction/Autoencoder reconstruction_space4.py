...
if training:
    ...
    for epoch in range(num_epochs):
        ...
        # plt.pause(0.1)
        # Save figure
        os.makedirs('autoencoder_progress_%d_features_final_4' % encoded_space_dim, exist_ok=True)
        plt.savefig('autoencoder_progress_%d_features_final_4/epoch_%d.png' % (encoded_space_dim, epoch + 1))
        plt.show()
        plt.close()
        torch.save(net.state_dict(), 'net_params_final4.pth')
...
net.load_state_dict(torch.load('net_params_final4.pth', map_location=device))
net.to(device)
val_loss = test_epoch(net, dataloader=test_dataloader, loss_fn=loss_fn, optimizer=optim) 
